...
kubectl wait --for=delete pod/tester --timeout=60s
...
sleep 5
kubectl wait --for=condition=containersready pod/tester --timeout=60s

etcdctl() {
  kubectl exec pod/tester -c etcdctl -- bash -c "ETCDCTL_API=3 etcdctl --endpoints='${ENDPOINTS}' ${1}"
}
...
scale_cluster() {
  ...
  kubectl wait --for=jsonpath='{.status.updatedReplicas}'="$1" sts/$CLUSTER_NAME --timeout=5m
  # wait for the last container to be running
  kubectl wait --for=jsonpath='{.status.readyReplicas}'="$1" sts/$CLUSTER_NAME --timeout=5m
